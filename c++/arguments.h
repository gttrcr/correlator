...
public:
    ...
    unsigned int codomain_column_index = get_default_codomain_column_index();                   // -c index of the column in the codomain scope to use in correlator
    ...
    static arguments get_arguments(int argc, char *argv[])
    {
        ...
        for (const auto &pair : complete_args)
        {
            ...
            if (pair.first == "-c" && pair.second.size() == 1 && utils::is_integer(pair.second[0]))
                args.codomain_column_index = std::stoi(pair.second[0]);
